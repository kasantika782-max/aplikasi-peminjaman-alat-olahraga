...
// Registrasi mandiri boleh dilakukan tanpa login.
// Semua aksi pengelolaan user lainnya hanya untuk Admin.
if ($aksi !== 'tambah' && ($_SESSION['role'] ?? '') !== 'admin') {
    $role_now = $_SESSION['role'] ?? '';
    if ($role_now === 'petugas') {
        header("Location: ../view/v_peminjaman_petugas.php");
    } elseif ($role_now === 'peminjam') {
...
if ($aksi === 'tampil') {
    ...
    if (basename($_SERVER['SCRIPT_FILENAME'] ?? '') === 'v_tampilan_user.php') {
        return;
    }
...
elseif ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Role tidak boleh dipercaya dari request. Registrasi publik selalu menjadi peminjam.
    $is_admin = (($_SESSION['role'] ?? '') === 'admin');
    $role     = $is_admin ? ($_POST['role'] ?? 'peminjam') : 'peminjam';
